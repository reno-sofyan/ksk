...
const VISUAL_PLACEHOLDER = ({ label, className = '' }) => (
  <div className={`flex min-h-56 items-center justify-center border border-dashed border-[#D4AF56]/70 bg-[linear-gradient(135deg,#2B0508,#65131C)] p-8 text-center shadow-[inset_0_0_80px_rgba(212,175,86,0.08)] ${className}`}>
    <div>
      ...
      <p className="mt-2 text-xs text-[#D8C7AA]">Aset visual Royal Kinara belum dikonfirmasi</p>
    </div>
  </div>
);
...
const RoyalRukoContent = () => {
  ...
  return (
    <div className="royal-content min-h-screen overflow-x-hidden bg-[#F8F3EA] pb-20 text-[#220306] md:pb-0" style={{ '--royal-maroon': MAROON }}>
      ...
      {!mobileMenuOpen ? <a href="#konsultasi" onClick={(event) => { event.preventDefault(); scrollToLead(); }} className="fixed bottom-[calc(1rem+env(safe-area-inset-bottom))] right-4 z-[70] inline-flex h-14 w-14 items-center justify-center rounded-full border border-[#D4AF56] bg-[#220306] text-[#E8CF8A] shadow-[0_18px_42px_rgba(34,3,6,0.35)] sm:right-6" aria-label="Buka form konsultasi Royal Kinara"><MessageCircle className="h-7 w-7" /></a> : null}
