...
const nearbyFacilities = [
  ...
  { label: 'Heroes Fitnes, Home Fitnes, Hilzastro Gym' },
  { label: 'Golden Sport Bogor, Dramaga Futsal, Vinci Mini Soccer' },
  { label: 'Spot Kuliner Bogor', image: imageUrl('mie gacoan.webp') },
  { label: 'Terminal Bubulak & Terminal Laladon' }
];

const photoFacilities = nearbyFacilities.filter((facility) => facility.image);
const textFacilities = nearbyFacilities.filter((facility) => !facility.image);
    ...
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
      {textFacilities.map((facility, index) => (
        <motion.div
          ...
          className="group flex min-h-24 items-center gap-3 rounded-lg border border-primary/15 bg-card px-4 py-4 text-left shadow-[0_10px_30px_rgba(7,39,29,0.06)] transition-all duration-300 hover:-translate-y-0.5 hover:border-accent/50 hover:shadow-[0_16px_34px_rgba(7,39,29,0.11)]"
        >
          ...
          <span className="text-sm font-bold leading-snug text-primary sm:text-base">{facility.label}</span>
        </motion.div>
      ))}
    </div>
  </div>
